      ...
      (let (
        ...
        (let ((reward-streak (if (or
          ...
          (let ((reward-amount (calculate-reward reward-streak)))
            ...
            (let ((faucet-balance (unwrap-panic (contract-call? .token get-balance CONTRACT))))
              (asserts! (>= faucet-balance reward-amount)
                err-insufficient-faucet-balance
              )
              ;; Transfer tokens from faucet to claimer
              (match (as-contract (contract-call? .token transfer reward-amount tx-sender claimer
                none
              ))
                success (begin
...
(define-public (deposit-tokens (amount uint))
  (begin
    ...
    (match (contract-call? .token transfer amount tx-sender CONTRACT none)
      success (begin
...
(define-read-only (get-faucet-balance)
  (unwrap-panic (contract-call? .token get-balance CONTRACT))
)
